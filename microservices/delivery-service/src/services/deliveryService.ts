...
export class DeliveryService {
    ...
    private async setupDeliveryConsumer() {
        await subscribeToTopic(this.deliveryConsumer, eventTypes.DELIVERY_EVENT, (payload) => {
            const deliveryData = JSON.parse(payload.message.value.toString()) as DeliveryModel;
