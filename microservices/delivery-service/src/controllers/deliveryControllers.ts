...
export const handleDeliveryRequest = async (req: Request, res: Response) => {
    ...
    await sendMessage('delivery-events', eventTypes.DELIVERY_EVENT, JSON.stringify({ orderId, deliveryAddress, deliveryDate }));

    res.status(202).json({ message: 'Delivery request forwarded for processing' });
};
